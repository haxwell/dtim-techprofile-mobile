		...
		self._route.params.subscribe((params) => {
			...
			self._functionPromiseService.initFunc(self.funcKey, () => new Promise((resolve, reject) => {
					self._userTechProfileModel.waitingPromise().then(() => {
						resolve({
							getEnv: () => environment,
							getColorMeaningString: () => 'lightblue means this user\'s score says they have demonstrated skills at this level, and below. White, user has not been tested, or has not passed, at this level.',
							getBackgroundColor: (lineItem, idx) => {
								const score = self._userTechProfileModel.getScore(lineItem.id);
